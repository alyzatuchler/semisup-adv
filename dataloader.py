...
def get_cifar10_vs_ti_loader(
    ...
    ti_data = np.memmap(
        ti_path, mode="r", dtype="uint8", order="F", shape=(32, 32, 3, 79302017)
    ).transpose([3, 0, 1, 2])

    logger.info("Size of tiny images {}".format(ti_data.size))
    ti_indices_path = os.path.join(dataset_dir, "ti_vs_cifar_inds.pickle")
...
def get_cifar100_vs_ti_loader(
    ...
    for dataset, name in zip((train_dataset, test_dataset), ("train", "test")):
        dataset.data = np.concatenate((dataset.data, ti_data[ti_indices[name]]))
        # All tinyimages are given label 10
        dataset.targets.extend([100] * len(ti_indices[name]))

    logger.info("Calling train sampler")
    # Balancing training batches with CIFAR100 and TI
    train_sampler = BalancedSampler(
        train_dataset.targets,
        batch_size,
        balanced_fraction=cifar_fraction,
