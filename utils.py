...
# Above not used, just defined for reference
...
# were obtained?
def load_tinyimage_subset(other_data_path, version_string="v7"):
    image_data_filename = "tinyimage_subset_data"
    if version_string != "":
        image_data_filename += "_" + version_string
    image_data_filename += ".pickle"
    image_data_filepath = os.path.abspath(
        os.path.join(other_data_path, image_data_filename)
    )
...
def load_cifar10_by_keyword(unique_keywords=True, version_string="v7"):
    cifar10_keywords = load_cifar10_keywords(
...
def load_new_test_data_indices(version_string="v7"):
    ...
    ti_indices_data_path = os.path.join(
