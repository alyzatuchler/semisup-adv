...
from datasets import DATASETS, SemiSupervisedDataset
from smoothing import Smooth
from utils import get_model

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Randomized smoothing certification")
    parser.add_argument(
        "--dataset", type=str, default="cifar10", choices=DATASETS, help="The dataset"
    )
    parser.add_argument(
        "--num_classes",
        type=int,
    ...
    parser.add_argument("--model_path", help="Model for certification")
    parser.add_argument(
        "--model", "-m", default="wrn-28-10", type=str, help="Name of the model"
    )
    parser.add_argument(
        "--output_name", default="smoothing", type=str, help="Name of output files"
    )
    parser.add_argument(
        "--sigma", default=0.25, type=float, help="Noise hyperparameter"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Skip this many examples between each examples " "we certify",
    )
    parser.add_argument(
        "--max", type=int, default=-1, help="Stop when example index == max"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of noise sample for classification " "decision",
    )
    parser.add_argument(
        "--N",
        type=int,
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(output_dir, "%s.log" % args.output_name)),
    ...
    base_classifier.load_state_dict(state_dict, strict=(args.model != "resnet-110"))
    ...
    if os.path.exists(os.path.join(output_dir, filename)):
        ...
        is_rob_correct = list(df.correct.values * (df.radius.values >= args.sigma))
    else:
        i_start = 0
        is_correct = []
        is_rob_correct = []
