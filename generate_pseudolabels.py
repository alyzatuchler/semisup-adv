...
parser.add_argument(
    "--model_dir", type=str, help="path of checkpoint to standard trained model"
)
parser.add_argument(
    "--model_epoch", "-e", default=200, type=int, help="Number of epochs trained"
)
parser.add_argument(
    "--data_dir", default="data/", type=str, help="directory that has unlabeled data"
)
parser.add_argument(
    "--data_filename",
    default="ti_top_50000_pred_v3.1.pickle",
    type=str,
    help="name of the file with unlabeled data",
)
parser.add_argument(
    "--output_dir", default="data/", type=str, help="directory to save output"
)
parser.add_argument(
    "--output_filename",
    default="pseudolabeled-top50k.pickle",
    type=str,
...
checkpoint = torch.load(
    os.path.join(args.model_dir, "checkpoint-epoch%d.pt" % args.model_epoch)
)
...
elif args.database == "cifar100":
    unlabeled_data = CIFAR100("data", train=False, transform=ToTensor())
else:
    sys.exit()
...
data_loader = torch.utils.data.DataLoader(
    unlabeled_data, batch_size=1000, num_workers=100, pin_memory=True
)
