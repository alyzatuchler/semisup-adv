...
def train(epoch, model, optimizer, scheduler, criterion, train_loader, run_config):
    global global_step
...
def main():
    ...
    criterion = nn.CrossEntropyLoss(
        reduction="mean", weight=torch.Tensor([1] * 10 + [0.1])
    ).cuda()
    ...
    for epoch in range(1, optim_config["epochs"] + 1):
        train_log = train(
            epoch, model, optimizer, scheduler, criterion, train_loader, run_config
        )
