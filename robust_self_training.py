...
from losses import noise_loss, trades_loss
...
parser.add_argument(
    "--data_dir", default="data", type=str, help="Directory where datasets are located"
)
parser.add_argument(
    "--svhn_extra", action="store_true", default=False, help="Adds the extra SVHN data"
)
...
parser.add_argument(
    "--save_freq", default=25, type=int, help="Checkpoint save frequency (in epochs)"
)
...
parser.add_argument(
    ...
    type=int,
    default=4,
    help="# of workers to use when loading in data into memory"
)
# Eval config
parser.add_argument(
    "--eval_freq", default=1, type=int, help="Eval frequency (in epochs)"
)
parser.add_argument(
    "--train_eval_batches",
    default=None,
...
parser.add_argument(
    "--nesterov", action="store_true", default=True, help="Use extragrdient steps"
)
...
parser.add_argument(
    "--pgd_step_size",
    default=0.007,
    help="pgd steps size in adversarial training",
    type=float,
)
parser.add_argument(
    "--beta",
...
parser.add_argument(
    ...
    default=None,
    type=int,
    help="Number of random aux examples to retain. " "None retains all aux data.",
)
...
parser.add_argument(
    "--entropy_weight", type=float, default=0.0, help="Weight on entropy loss"
)
...
if args.autoaugment or args.cutout:
    ...
    if args.autoaugment:  # Assume CIFAR10 policy applies to CIFAR100
    ...
    logger.info("Applying aggressive training augmentation: %s" % transform_train)
...
eval_train_loader = DataLoader(
    trainset_eval, batch_size=args.test_batch_size, shuffle=True, **kwargs
)
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if args.loss == "trades":
            ...
            loss = noise_loss(
                model=model, x_natural=data, y=target, clamp_x=True, epsilon=epsilon
            )
            entropy_loss_unlabeled = torch.Tensor([0.0])
...
def eval(args, model, device, eval_set, loader):
        ...
        for batch_idx, (data, target) in enumerate(loader):
            ...
            if batch_idx < args.eval_attack_batches:
                if args.distance == "l_2":
                    ...
                    raise ValueError("No support for distance %s", args.distance)
                adv_correct_clean += len(data) - int(incorrect_clean)
                adv_correct += len(data) - int(incorrect_rob)
                adv_total += len(data)
            total += len(data)
            if (eval_set == "train") and (batch_idx + 1 == args.train_eval_batches):
                break
    loss /= total
    ...
    if adv_total > 0:
        ...
        robust_accuracy = robust_clean_accuracy = 0.0
    ...
    logging.info(
        ...
        "Robust accuracy {}/{} ({:.2f}%)".format(
            ...
            adv_correct_clean,
            adv_total,
            100.0 * robust_clean_accuracy,
            adv_correct,
            adv_total,
            100.0 * robust_accuracy,
        )
    )
...
def main():
    ...
    model = get_model(
        args.model, num_classes=num_classes, normalize_input=args.normalize_input
    )
    if use_cuda:
