...
def eval_adv_test(
    model, device, test_loader, attack, attack_params, results_dir, num_eval_batches
):
    """
    ...
    if attack == "pgd":
        ...
        for restart in range(attack_params["num_restarts"]):
            ...
            num_correct_adv = (
                is_correct_adv_over_restarts.prod(axis=-1).prod(axis=-1).sum()
            )

            logging.info(
                "Accuracy after %d restarts: %.4g%%"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_suffix", default="", type=str, help="String to add to log filename"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--epsilon", default=0.031, type=float, help="Attack perturbation magnitude"
    )
    parser.add_argument(
        "--attack",
        default="pgd",
    ...
    parser.add_argument(
        "--num_restarts", default=5, type=int, help="Number of restarts for PGD attack"
    )
    parser.add_argument(
        "--no_random_start",
        dest="random_start",
    ...
    parser.add_argument(
        "--learning_rate", default=5e-3, type=float, help="Learning rate for CW attack"
    )
    parser.add_argument(
        "--initial_const",
        default=1e-2,
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                os.path.join(
                    output_dir, "attack_epoch%d%s.log" % (epoch, args.output_suffix)
                )
            ),
    ...
    test_loader = torch.utils.data.DataLoader(
        testset, batch_size=args.batch_size, shuffle=False, **dl_kwargs
    )
    ...
    attack_params = {"epsilon": args.epsilon, "seed": args.random_seed}
    if args.attack == "pgd":
