...
parser.add_argument(
    "--num_workers", type=int, default=80, help="Number of workers for data loading"
)
...
class MemmapDataset(Dataset):
    ...
    def __getitem__(self, item):
        return to_tensor(self.data[item], self.data.dtype)
    ...
    def __init__(self, metric, target_set):
        ...
        if metric == "l2":
            ...
            self.register_buffer(
                "target_set_norm_squared", (target_set ** 2).sum(dim=0, keepdim=True)
            )
        elif metric == "cosine":
...
def main():
    ...
    if os.path.exists(out_path):
        ...
        # loading data
        data = np.memmap(data_path, mode="r", dtype=data_dtype, order="C").reshape(
            num_images, -1
        )
    ...
    if os.path.exists(indices_path):
        ...
        num_ti_test = int(1e4)
        ti_indices = valid_indices[
            np.random.permutation(num_valid)[: (num_ti_train + num_ti_test)]
        ]
        # save indices
        indices_path = os.path.join(args.output_dir, "ti_vs_cifar_inds.pickle")
        with open(indices_path, "wb") as f:
            pickle.dump(
                dict(train=ti_indices[:num_ti_train], test=ti_indices[num_ti_train:]), f
            )
