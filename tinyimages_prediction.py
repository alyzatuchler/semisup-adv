...
# Model details
parser.add_argument(
    "--model_path", type=str, help="path of checkpoint to pretrained model"
)
parser.add_argument(
    "--model", "-m", default="wrn-28-10", type=str, help="name of the model"
)
# Directory details
parser.add_argument(
...
parser.add_argument(
    "--output_dir", type=str, help="directory where unlabeled dataset is to be located"
)
parser.add_argument(
    "--output_filename",
    type=str,
...
# Running details
parser.add_argument(
    "--batch_size", type=int, default=1000, metavar="N", help="batch_size"
)
parser.add_argument(
    "--save_freq",
    "-s",
...
logits_file = np.memmap(
    logits_path,
    dtype=np.float32,
    mode=_get_mode(logits_path),
...
if args.save_prelogits:
    ...
    prelogits_file = np.memmap(
        prelogits_path,
        dtype=np.float32,
        mode=_get_mode(prelogits_path),
...
for i, batch in enumerate(data_loader):
    ...
    with torch.no_grad():
        ...
        output_batches.append((logits.cpu().numpy(), prelogits.cpu().numpy()))

    # Saving data
    if (i + 1) % args.save_freq == 0 or i == num_batches - 1:
        logits_batch, prelogits_batch = [
            np.concatenate(s) for s in zip(*output_batches)
        ]
        ...
        logging.info(
            "Processed %d/%d images (%.4g%%), %.3g images/sec"
            % (count, num_images, 100 * count / num_images, increment / elapsed)
        )
...
for label in labels:
    ...
    inds = np.concatenate(
        (
            inds,
            np.where(mask)[0][
                np.argsort(ti_prediction_probs[mask])[-args.num_unlabeled :]
            ],
        )
    )

data = np.array(data[inds])
targets = np.concatenate([args.num_unlabeled * [label] for label in labels]).astype(
    "int"
)
out = dict(data=data, extrapolated_targets=targets, ti_index=inds)

print(
    "Created %s (%d images from %d candidates)"
    % (args.output_filename, len(data), is_valid.sum())
)
