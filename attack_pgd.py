...
def pgd(model, X, y, epsilon=8 / 255, num_steps=20, step_size=0.01, random_start=True):
    out = model(X)
    ...
    for _ in range(num_steps):
        ...
        perturbation.data = (
            perturbation + step_size * perturbation.grad.detach().sign()
        ).clamp(-epsilon, epsilon)
        perturbation.data = torch.min(
            torch.max(perturbation.detach(), -X), 1 - X
        )  # clip X+delta to [0,1]
        X_pgd = Variable(
            torch.clamp(X.data + perturbation.data, 0, 1.0), requires_grad=False
        )
        is_correct_adv.append(
            np.reshape((model(X_pgd).max(1)[1] == y).float().cpu().numpy(), [-1, 1])
        )
